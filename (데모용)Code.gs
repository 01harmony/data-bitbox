 ...
 *      (그 URL과 SECRET_KEY 값은 "링크_생성기.html"에 입력해서, 학생들에게
 *       나눠줄 링크를 만드는 데 사용합니다.)
 * ==================================================================
 */

// 🔑 [필수 수정 1] 학생 제출용 비밀값. 나만 아는 값으로 바꾸세요.
//    이 값은 깃허브(공개 저장소)에는 노출되지 않습니다 — "링크_생성기.html"에서 학생용
//    링크를 만들 때만 쓰이고, 그 링크를 통해 학생 브라우저에 저장됩니다.
//    → 학생이 "제출"할 때만 쓰이는 값이라고 생각하면 됩니다.
const SECRET_KEY = "bitbox-2026";

// 🔑 [필수 수정 2] 선생님(관리자) 전용 비밀값. SECRET_KEY와 반드시 다른 값으로 바꾸세요.
//    이 값은 어떤 파일에도 저장하지 않습니다 — 대시보드에 처음 연결할 때 선생님이 직접
//    입력하면, 그다음부터는 같은 브라우저에서만 자동으로 재연결됩니다.
